...
import type { FavoriteTeam } from "./authClient";
import { mapBclRawGame } from "./bclMapper";
import { mapPlgRawGame } from "./plgMapper";
import { getCanonicalFavoriteTeamId, resolveTeamId } from "./teamIdentity";
import { mapFallbackTpblGame } from "./tpblMapper";
...
export function getFavoriteTeamGamesByDate(
  ...
  const tpblGames: LeagueGame[] = (tpblScheduleData.games as TpblFallbackGame[]).map(
    (game) => ({
  ...
  return [...plgGames, ...tpblGames, ...bclGames]
    ...
    .filter((game) => getGameTeamKeys(game).some((teamKey) => favoriteKeys.has(teamKey)))
